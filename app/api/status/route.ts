...
export async function GET(req: NextRequest) {
  ...
  const statusUrl = job?.falStatusUrl || `https://queue.fal.run/${FAL_MODEL}/requests/${falId}/status`;
  const resultUrl = job?.falResultUrl || `https://queue.fal.run/${FAL_MODEL}/requests/${falId}`;
  ...
  try {
    ...
    if (statusData.status === "COMPLETED") {
      ...
      if (videoUrl) {
        if (job) {
          try { await store.updateJob(job.id, { status: "completed", resultVideoUrl: videoUrl, completedAt: Date.now() }); } catch {}
        }
