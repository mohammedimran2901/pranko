...
function rowToJob(row: any): PrankJob {
  return {
    ...
    falStatusUrl: row.fal_status_url ?? undefined,
    falResultUrl: row.fal_result_url ?? undefined,
    ...
    if (hasSupabaseEnv()) {
      try {
        ...
        const { error } = await supabase.from(JOBS_TABLE).insert({
          ...
          fal_status_url: params.statusUrl ?? null,
          fal_result_url: params.resultUrl ?? null,
